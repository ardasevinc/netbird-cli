...
Peer ingress-port allocation creation, updates, and deletion are capability-gated, blocking, and require peer-scoped exact preimage plus read-back or absence confirmation because they can expose or remove external service mappings.
...
Setup-key updates and deletion are capability-gated and blocking, with enrollment-impact evidence, exact preimage verification, and read-back or absence confirmation; setup-key creation remains one-time-secret gated.
...
Reverse-proxy service creation, updates, deletion, and account-cluster deletion are capability-gated and blocking because they change public ingress infrastructure. Service auth is supplied through an external `auth_ref` and resolved only in memory at dispatch.
...
go run ./cmd/nb ingress-ports list <peer-id> --json
