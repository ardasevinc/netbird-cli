...
func (c *Client) GetNameserverGroupRaw(ctx context.Context, id string) (json.RawMessage, error) {
	path, err := dnsPath("nameservers", id)
	if err != nil {
		return nil, err
	}
	var result json.RawMessage
	if _, err := c.transport.DoJSON(ctx, http.MethodGet, path, nil, &result); err != nil {
		return nil, fmt.Errorf("get nameserver group %q: %w", id, err)
	}
	return result, nil
}

func (c *Client) UpdateNameserverGroup(ctx context.Context, id string, request json.RawMessage) (json.RawMessage, error) {
	path, err := dnsPath("nameservers", id)
	if err != nil {
		return nil, err
	}
	...
	if _, err := c.transport.DoJSON(ctx, http.MethodPut, path, request, &result); err != nil {
		return nil, fmt.Errorf("update nameserver group %q: %w", id, err)
	}
	return result, nil
}
