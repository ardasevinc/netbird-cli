...
func TestUpdateDNSRecordUsesPUT(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPut || r.URL.Path != "/api/dns/zones/zone-1/records/record-1" {
			t.Fatalf("unexpected request: %s %s", r.Method, r.URL.Path)
		}
		var request map[string]any
		if err := json.NewDecoder(r.Body).Decode(&request); err != nil {
		...
		if request["content"] != "10.0.0.6" {
			t.Fatalf("unexpected request: %#v", request)
		}
		_ = json.NewEncoder(w).Encode(map[string]any{"id": "record-1", "name": "db", "type": "A", "content": "10.0.0.6", "ttl": 60})
	}))
	defer server.Close()
	transportClient, err := transport.New(transport.Config{BaseURL: server.URL, HTTP: server.Client()})
	if err != nil {
	...
	response, err := NewClient(transportClient).UpdateDNSRecord(context.Background(), "zone-1", "record-1", json.RawMessage(`{"content":"10.0.0.6"}`))
	if err != nil {
		t.Fatal(err)
	}
	var updated map[string]any
	if err := json.Unmarshal(response, &updated); err != nil || updated["content"] != "10.0.0.6" {
		t.Fatalf("updated=%s err=%v", response, err)
	}
}
